...
struct InstrumentArgs {
    ...
    symbol: String,
    #[serde(default)]
    #[schemars(description = "Tracing form")]
    r#with: Option<String>,
    #[serde(default)]
    #[schemars(description = "Sub-expression")]
    at: Option<String>,
    #[serde(default)]
    #[schemars(description = "Wrapper")]
    wrap: Option<String>,
    #[serde(default)]
    write: bool,
}
...
fn tool_result(r: Result<String, lisp_sitter_core::Error>) -> Result<String, String> {
    match r {
        Ok(t) => Ok(t),
...
/// Shared write/diff/preview path for MCP edits that compute an updated buffer first.
fn finish_mcp_edit(
    path: &str,
    original: &str,
    updated: &str,
    ...
    if diff {
        let d = ops::diff_text(original, updated, path);
        if !d.is_empty() {
            out.push_str(&d);
        }
    }
    if write {
        ops::atomic_write(path, updated).map_err(|e| e.to_string())?;
        if !out.is_empty() {
            out.push('\n');
        }
...
impl LispSitterMcp {
    #[tool(
        description = "Validate a whole structural Lisp file. Pass semantic=true for deep validation."
    )]
    async fn check_structural_file(
        &self,
        Parameters(args): Parameters<PathArgs>,
    ) -> Result<String, String> {
        if args.semantic {
            tool_result(ops::check_semantic(&self.reg, &args.path))
        } else {
            tool_result(ops::check_structural_file(&self.reg, &args.path))
        }
    }

    #[tool(
        description = "List MISSING tokens and ERROR nodes in a Lisp file as reported by tree-sitter. Use this to locate missing parentheses or other structural problems without reading the whole file."
    )]
    async fn structural_find_errors(
        &self,
        Parameters(args): Parameters<PathArgs>,
    ) -> Result<String, String> {
        tool_result(ops::find_errors(&self.reg, &args.path))
    }

    #[tool(description = "Validate a complete top-level node without saving.")]
    async fn check_structural_node(
        &self,
        Parameters(args): Parameters<PathNodeArgs>,
    ) -> Result<String, String> {
        tool_result(ops::check_structural_node(
            &self.reg, &args.path, &args.node,
        ))
    }

    #[tool(
        description = "Outline of top-level forms. Set depth > 1 for sub-form navigation. \
                       Set all=true to include non-definition forms (require, provide, setq, …)."
    )]
    async fn structural_tree(
        &self,
        Parameters(args): Parameters<PathDepthArgs>,
    ) -> Result<String, String> {
        ...
        if args.all {
            ...
            tool_result(ops::tree(&self.reg, &args.path))
        }
    }

    #[tool(description = "Return byte positions START:END for a named top-level form.")]
    async fn structural_bounds(
        &self,
        Parameters(args): Parameters<PathSymbolArgs>,
    ) -> Result<String, String> {
        tool_result(ops::bounds(&self.reg, &args.path, &args.symbol))
    }

    #[tool(
        description = "Replace one top-level form with complete new text. Pass write=true to save; \
                       without it returns the updated file content (or a diff when diff=true)."
    )]
    async fn structural_replace(
        ...
        let u = lisp_sitter_core::edit::replace_node(p, &c, &args.symbol, &args.new_body)
            .map_err(|e| e.to_string())?;
        finish_mcp_edit(&args.path, &c, &u, args.write, args.diff)
    }

    #[tool(
        description = "Insert a complete top-level form after __start__, __end__, or a symbol name. \
                       Pass write=true to save; without it returns the updated file content \
                       (or a diff when diff=true)."
    )]
    async fn structural_insert(
        &self,
    ...
    async fn structural_get(
        &self,
        Parameters(args): Parameters<PathSymbolArgs>,
    ) -> Result<String, String> {
        tool_result(ops::get_form(&self.reg, &args.path, &args.symbol))
    }

    #[tool(
        description = "Complete an unbalanced s-expression by appending missing closing parens."
    )]
    async fn structural_complete(
        &self,
        Parameters(args): Parameters<CompleteArgs>,
    ) -> Result<String, String> {
        tool_result(ops::complete_node(&self.reg, &args.lang, &args.body))
    }

    #[tool(description = "Return the complete structural context: outline, bounds, and full text.")]
    async fn structural_context(
        &self,
        Parameters(args): Parameters<PathArgs>,
    ) -> Result<String, String> {
        ...
        match lisp_sitter::eval::eval_file(&args.path) {
            Ok((s, e, ok)) => {
                ...
                if ok {
                    ...
                    Err(if r.is_empty() { "error".into() } else { r })
                }
            }
            Err(e) => Err(e.to_string()),
        }
    ...
    #[tool(
        description = "Re-indent a file (depth-based). Pass write=true to save. \
                       Pass align=true to align continuation lines to the first argument."
    )]
    async fn structural_format(
        &self,
        Parameters(args): Parameters<FormatArgs>,
    ) -> Result<String, String> {
        let c = ops::read_file(&args.path).map_err(|e| e.to_string())?;
        let f = if args.align {
            lisp_sitter_core::format_source_aligned(&c)
        } else {
            ops::format_content(&c, &self.reg, &args.path).map_err(|e| e.to_string())?
        };
        let mut r = String::new();
        if args.diff {
            let d = ops::diff_text(&c, &f, &args.path);
            if !d.is_empty() {
                r.push_str(&d);
            }
        }
        if args.write {
            let p = ops::resolve_plugin(&self.reg, &args.path, None).map_err(|e| e.to_string())?;
            lisp_sitter_core::edit::ensure_source_editable(p, &c).map_err(|e| e.to_string())?;
            p.check_file(&f).map_err(|e| e.to_string())?;
            ops::atomic_write(&args.path, &f).map_err(|e| e.to_string())?;
            r.push_str(&format!("Wrote {}", args.path));
        } else if !args.diff {
            r.push_str(&f);
        }
        Ok(r)
    ...
    async fn structural_rename(
        &self,
        Parameters(args): Parameters<RenameArgs>,
    ) -> Result<String, String> {
        let ref_mode = if args.no_refs {
            lisp_sitter::transform::RefsMode::HeadOnly
        } else if args.refs {
            lisp_sitter::transform::RefsMode::AllRefs
        } else {
            lisp_sitter::transform::RefsMode::HeadAndSharp
        };
        let u =
            lisp_sitter::transform::rename(&self.reg, &args.path, &args.old, &args.new, ref_mode)
                .map_err(|e| e.to_string())?;
        if args.write {
    ...
    #[tool(
        description = "Rename a symbol across an entire project. `path` is a directory or glob. \
    ...
    async fn structural_rename_project(
        &self,
    ...
    #[tool(
        description = "Project-wide semantic analysis over a directory or glob: unused definitions, \
    ...
    async fn structural_analyze(
        &self,
        Parameters(args): Parameters<AnalyzeArgs>,
    ) -> Result<String, String> {
        let opt = if !args.unused && !args.unresolved && !args.arity {
            lisp_sitter::analyze::Options::all()
        } else {
            lisp_sitter::analyze::Options {
                unused: args.unused,
    ...
    async fn structural_wrap(
        &self,
        Parameters(args): Parameters<WrapArgs>,
    ) -> Result<String, String> {
        let mut xs: Vec<(&str, &str)> = Vec::new();
        if let Some(ref b) = args.bindings {
            xs.push(("bindings", b));
        }
        ...
        let u =
            lisp_sitter::transform::wrap_body(&self.reg, &args.path, &args.symbol, &args.r#in, &xs)
                .map_err(|e| e.to_string())?;
        if args.write {
            ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?;
            Ok(format!("Wrote {}", args.path))
    ...
    async fn structural_remove(
        &self,
        Parameters(args): Parameters<RemoveArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::remove_form(
            &self.reg,
            &args.path,
            &args.symbol,
            args.keep_calls,
    ...
    async fn structural_move(
        &self,
        Parameters(args): Parameters<MoveArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::move_form(&self.reg, &args.path, &args.symbol, &args.after)
            .map_err(|e| e.to_string())?;
        if args.write {
            ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?;
            Ok(format!("Wrote {}", args.path))
    ...
    async fn structural_substitute(
        &self,
        Parameters(args): Parameters<SubstituteArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::substitute(
            &self.reg,
            &args.path,
            &args.symbol,
            &args.pattern,
    ...
    #[tool(
        description = "Paredit splice: remove a wrapper form, elevating its body into the \
        surrounding context (strips both the outer parens and the head symbol). \
        Example: `(when cond (progn (a) (b)))` splice `(progn (a) (b))` → `(when cond (a) (b))`. \
        Pass write=true to save."
    )]
    async fn structural_splice(
        &self,
        Parameters(args): Parameters<PatternArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::splice(&self.reg, &args.path, &args.symbol, &args.pattern)
            .map_err(|e| e.to_string())?;
        if args.write {
            ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?;
            Ok(format!("Wrote {}", args.path))
    ...
    #[tool(
        description = "Paredit raise: replace the direct enclosing list with a chosen \
        sub-expression. \
        Example: `(if cond (bar x) nil)` raise `(bar x)` → `(bar x)`. \
        Pass write=true to save."
    )]
    async fn structural_raise(
        &self,
        Parameters(args): Parameters<PatternArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::raise(&self.reg, &args.path, &args.symbol, &args.pattern)
            .map_err(|e| e.to_string())?;
        if args.write {
            ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?;
            Ok(format!("Wrote {}", args.path))
    ...
    async fn structural_extract(
        ...
        let params: Vec<&str> = args
            ...
            .filter(|s| !s.is_empty())
            .collect();
        let u = lisp_sitter::transform::extract(
            &self.reg,
            &args.path,
            &args.symbol,
            &args.pattern,
    ...
    async fn structural_callers(
        &self,
        Parameters(args): Parameters<PathSymbolArgs>,
    ) -> Result<String, String> {
        tool_result(lisp_sitter::graph::callers(
            &self.reg,
            &args.path,
            &args.symbol,
        ))
    ...
    #[tool(
        description = "Symbols called directly from a definition's body across path (directory or glob)."
    )]
    async fn structural_callees(
        &self,
        Parameters(args): Parameters<PathSymbolArgs>,
    ) -> Result<String, String> {
        tool_result(lisp_sitter::graph::callees(
            &self.reg,
            &args.path,
            &args.symbol,
        ))
    ...
    async fn structural_explore(
        &self,
        Parameters(args): Parameters<PathSymbolArgs>,
    ) -> Result<String, String> {
        tool_result(lisp_sitter::graph::explore(
            &self.reg,
            &args.path,
            &args.symbol,
        ))
    ...
    async fn structural_impact(
        &self,
        Parameters(args): Parameters<ImpactArgs>,
    ) -> Result<String, String> {
        let depth = args
            .depth
            .unwrap_or(lisp_sitter::graph::default_impact_depth() as i64)
            .max(0) as usize;
        tool_result(lisp_sitter::graph::impact(
            &self.reg,
            &args.path,
            &args.symbol,
            depth,
    ...
    #[tool(
        description = "Map git changes since ref to touched symbols in path. Set impact=true for blast radius per symbol."
    )]
    async fn structural_diff(
        ...
        let depth = args
            ...
            .unwrap_or(lisp_sitter::graph::default_impact_depth() as i64)
            .max(0) as usize;
        tool_result(lisp_sitter::graph::diff(
            &self.reg,
            &args.path,
            &args.r#ref,
            depth,
    ...
    #[tool(
        description = "Instrument a form with tracing (--with for body, --at/--wrap for sub-expressions)."
    )]
    async fn structural_instrument(
        &self,
        Parameters(args): Parameters<InstrumentArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::instrument(
            &self.reg,
            &args.path,
            &args.symbol,
            args.r#with.as_deref(),
            args.at.as_deref(),
            args.wrap.as_deref(),
        )
        .map_err(|e| e.to_string())?;
    ...
    async fn structural_convert_let(
        &self,
        Parameters(args): Parameters<ConvertLetArgs>,
    ) -> Result<String, String> {
        let u = lisp_sitter::transform::convert_let(&self.reg, &args.path, &args.symbol, &args.to)
            .map_err(|e| e.to_string())?;
        if args.write {
            ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?;
            Ok(format!("Wrote {}", args.path))
...
mod tests {
    ...
    async fn test_check_structural_file() {
        ...
        let r = s
            .check_structural_file(Parameters(PathArgs {
    ...
    async fn test_check_structural_node() {
        ...
        let r = s
            .check_structural_node(Parameters(PathNodeArgs {
                path: p.to_str().unwrap().to_string(),
                node: "(defun foo () 1)".into(),
            }))
            .await;
        assert_eq!(r.unwrap(), "OK");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn test_structural_tree() {
        ...
        let r = s
            .structural_tree(Parameters(PathDepthArgs {
                path: p.to_str().unwrap().to_string(),
                depth: Some(1),
                all: false,
            }))
    ...
    async fn test_structural_bounds() {
        ...
        let r = s
            .structural_bounds(Parameters(PathSymbolArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
            }))
            .await;
    ...
    async fn test_structural_get() {
        ...
        let r = s
            .structural_get(Parameters(PathSymbolArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
            }))
            .await;
    ...
    async fn test_structural_complete() {
        let s = mcp();
        let r = s
            .structural_complete(Parameters(CompleteArgs {
                lang: "elisp".into(),
                body: "(defun foo (x)".into(),
            }))
            .await;
    ...
    #[tokio::test]
    async fn test_structural_find_errors_clean() {
        let s = mcp();
        let (dir, p) = tmp_el("find_errors_clean", "(defun foo () 1)\n");
        let r = s
            .structural_find_errors(Parameters(PathArgs {
                path: p.to_str().unwrap().to_string(),
                semantic: false,
            }))
            .await;
        assert_eq!(r.unwrap(), "No errors");
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[tokio::test]
    async fn test_structural_find_errors_missing_paren() {
        let s = mcp();
        let (dir, p) = tmp_el("find_errors_bad", "(defun foo ()\n");
        let r = s
            .structural_find_errors(Parameters(PathArgs {
                path: p.to_str().unwrap().to_string(),
                semantic: false,
        ...
        assert!(
            ...
            "expected missing paren report, got: {out}"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    async fn test_structural_context() {
        ...
        let r = s
            .structural_context(Parameters(PathArgs {
                path: p.to_str().unwrap().to_string(),
                semantic: false,
            }))
            .await;
    ...
    async fn test_structural_format() {
        ...
        let r = s
            .structural_format(Parameters(FormatArgs {
                path: p.to_str().unwrap().to_string(),
                write: false,
                diff: false,
                align: false,
    ...
    async fn test_structural_replace() {
        ...
        let r = s
            .structural_replace(Parameters(ReplaceArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
    ...
    #[tokio::test]
    async fn test_structural_replace_dry_run() {
        let s = mcp();
        let (dir, p) = tmp_el("replace_dry", "(defun foo () 1)\n");
        let r = s
            .structural_replace(Parameters(ReplaceArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
            ...
            .await
            .unwrap();
        assert!(r.contains("(defun foo () 42)"));
        let content = std::fs::read_to_string(&p).unwrap();
        assert_eq!(content, "(defun foo () 1)\n");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    async fn test_structural_insert() {
        ...
        let r = s
            .structural_insert(Parameters(InsertArgs {
                path: p.to_str().unwrap().to_string(),
    ...
    async fn test_structural_rename() {
        ...
        let r = s
            .structural_rename(Parameters(RenameArgs {
                refs: false,
                no_refs: false,
                path: p.to_str().unwrap().to_string(),
                old: "foo".into(),
    ...
    async fn test_structural_remove() {
        ...
        let r = s
            .structural_remove(Parameters(RemoveArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
                keep_calls: true,
                write: false,
    ...
    async fn test_structural_move() {
        ...
        let r = s
            .structural_move(Parameters(MoveArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "a".into(),
                after: "b".into(),
                write: false,
    ...
    async fn test_structural_substitute() {
        ...
        let r = s
            .structural_substitute(Parameters(SubstituteArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
                pattern: "(+ x 1)".into(),
    ...
    async fn test_structural_wrap() {
        ...
        let r = s
            .structural_wrap(Parameters(WrapArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
                r#in: "progn".into(),
                bindings: None,
    ...
    async fn test_structural_explore() {
        let s = mcp();
        let dir =
            std::env::temp_dir().join(format!("lisp-sitter-mcp-explore-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let p = dir.join("a.el");
        ...
        let r = s
            .structural_explore(Parameters(PathSymbolArgs {
    ...
    async fn test_structural_callers() {
        ...
        let r = s
            .structural_callers(Parameters(PathSymbolArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "b".into(),
            }))
            .await;
    ...
    async fn test_structural_flatten() {
        ...
        let r = s
            .structural_flatten(Parameters(FlattenArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "add1".into(),
                write: false,
            }))
    ...
    async fn test_structural_convert_let() {
        ...
        let r = s
            .structural_convert_let(Parameters(ConvertLetArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
                to: "let*".into(),
                write: false,
    ...
    async fn test_structural_instrument_with() {
        ...
        let r = s
            .structural_instrument(Parameters(InstrumentArgs {
                path: p.to_str().unwrap().to_string(),
                symbol: "foo".into(),
    ...
    async fn test_structural_eval_no_file() {
        let s = mcp();
        let r = s
            .structural_eval(Parameters(PathArgs {
                path: "/nonexistent.el".into(),
                semantic: false,
            }))
            .await;
        assert!(r.is_err());
