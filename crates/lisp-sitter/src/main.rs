...
async fn run(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Fmt { path, write, diff, align } => {
            let content = lisp_sitter::ops::read_file(&path)?; let f = if align { lisp_sitter_core::format_source_aligned(&content) } else { lisp_sitter::ops::format_file(&reg, &path)? };
            if diff || cf { let d = lisp_sitter::ops::diff_text(&content, &f, &path); if !d.is_empty() { eprint!("{d}"); } }
            if cf { confirm_or_abort(); }
            if write { reg.plugin_for_path(&path)?.check_file(&f)?; lisp_sitter::ops::atomic_write(&path, &f)?; println!("Wrote {path}"); } else { print!("{f}"); }
        }
