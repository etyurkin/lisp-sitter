...
impl Registry {
    pub fn new() -> Self { Self { plugins: Vec::new(), extra_exts: HashMap::new() } }

    pub fn register(&mut self, plugin: Box<dyn LanguagePlugin>) { self.plugins.push(plugin); }
    ...
    pub fn plugin_for_path(&self, path: &str) -> Result<&dyn LanguagePlugin> {
        ...
        for p in &self.plugins { if p.matches_path(path) { return Ok(p.as_ref()); } }
        // Custom extension mappings
        if let Some(lang_id) = self.extra_exts.iter().find(|(ext, _)| path.ends_with(ext.as_str())).map(|(_, id)| id) {
            for p in &self.plugins { if p.id() == lang_id.as_str() { return Ok(p.as_ref()); } }
    ...
    pub fn plugins(&self) -> &[Box<dyn LanguagePlugin>] { &self.plugins }
}
