...
enum Command {
    ...
    Tree { path: String, #[arg(long, default_value = "1")] depth: usize, #[arg(long, help = "Include non-definition top-level forms (require, provide, setq, …)")] all: bool },
    ...
    Fmt { path: String, #[arg(long)] write: bool, #[arg(long)] diff: bool, #[arg(long, help = "Align continuation lines to the first argument instead of depth×2")] align: bool },
...
enum McpCommand {
    ...
    Install { #[arg(long)] cursor: bool, #[arg(long, help = "Write to Claude Code CLI (~/.claude.json)")] claude_code: bool, #[arg(long, help = "Write to Claude Desktop (~/.claude/settings.json)")] claude_desktop: bool },
}
...
async fn run(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        Command::Tree { path, depth, all } => {
            ...
            else if all { println!("{}", lisp_sitter::ops::tree_depth(&reg, &path, depth.saturating_sub(1))?); }
        ...
        Command::Replace { path, symbol, body, body_file, write, diff } => {
            ...
            let u = lisp_sitter_core::edit::replace_node(lisp_sitter::ops::resolve_plugin(&reg, &path, None)?, &content, &symbol, &b)?;
            if diff || cf { let d = lisp_sitter::ops::diff_text(&content, &u, &path); if !d.is_empty() { eprint!("{d}"); } }
            if cf { confirm_or_abort(); }
        ...
        Command::Insert { path, after, node, node_file, write, diff } => {
            ...
            let u = lisp_sitter_core::edit::insert_after(lisp_sitter::ops::resolve_plugin(&reg, &path, None)?, &content, &after, &n)?;
            if diff || cf { let d = lisp_sitter::ops::diff_text(&content, &u, &path); if !d.is_empty() { eprint!("{d}"); } }
            if cf { confirm_or_abort(); }
            if write { lisp_sitter::ops::atomic_write(&path, &u)?; println!("Wrote {path}"); } else { print!("{u}"); }
        }
        ...
        Command::Fmt { path, write, diff, align } => {
            let content = lisp_sitter::ops::read_file(&path)?; let f = if align { lisp_sitter_core::format_source_aligned(&content) } else { lisp_sitter::ops::format_content(&content, &reg, &path)? };
            if diff || cf { let d = lisp_sitter::ops::diff_text(&content, &f, &path); if !d.is_empty() { eprint!("{d}"); } }
            if cf { confirm_or_abort(); }
            if write { let p = lisp_sitter::ops::resolve_plugin(&reg, &path, None)?; lisp_sitter_core::edit::ensure_source_editable(p, &content)?; p.check_file(&f)?; lisp_sitter::ops::atomic_write(&path, &f)?; println!("Wrote {path}"); } else { print!("{f}"); }
        }
