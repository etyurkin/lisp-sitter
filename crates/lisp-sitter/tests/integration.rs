...
fn elisp_realistic_works() {
    ...
    for def in [
        ...
        assert!(
            outline.contains(def),
            "missing from outline: {def}\n{outline}"
        );
    }
    ...
    assert!(reg
        .plugin_for_path(path.to_str().unwrap())
        .unwrap()
        .check_file(&updated)
        .is_ok());
    ...
    // ── check: file is valid ────────────────────────────────────
    assert_eq!(
        ops::check_structural_file(&reg, path.to_str().unwrap()).unwrap(),
        "OK"
    );
...
fn elisp_rename_string_safe() {
    ...
    let result = transform::rename(
        &reg,
        path.to_str().unwrap(),
        "foo",
        "baz",
        transform::RefsMode::HeadOnly,
    );
    assert!(result.is_ok());
    let got = result.unwrap();
    assert!(
        got.contains("\"call (foo) here\""),
        "string must be untouched: {got}"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn elisp_flatten_arg_substitution() {
    ...
    let got = result.unwrap();
    assert!(
        !got.contains("(defun add1"),
        "definition should be removed: {got}"
    );
    assert!(
        got.contains("(+ 2 1)"),
        "should inline with arg substitution: {got}"
    );
...
fn scheme_realistic_works() {
    ...
    let outline = ops::tree(&reg, path.to_str().unwrap()).unwrap();
    assert!(
        outline.contains("define-library"),
        "missing library form: {outline}"
    );
    assert!(
        outline.contains("my lib"),
        "missing library name: {outline}"
    );

    assert!(
        ops::bounds(&reg, path.to_str().unwrap(), "my lib").is_ok(),
...
fn elisp_rename_sharp_quote_refs() {
    ...
    let result = transform::rename(
        ...
        "foo",
        "baz",
        transform::RefsMode::HeadAndSharp,
    );
    assert!(result.is_ok());
    let got = result.unwrap();
...
fn elisp_rename_in_string_not_touched() {
    ...
    assert!(
        got.contains("\"call (foo) here\""),
        "string should be untouched: {got}"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn fmt_multiline_preserves_structure() {
    ...
    assert_eq!(
        ops::format_file(&reg, path.to_str().unwrap()).unwrap(),
        content
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn malformed_source_refuses_destructive_edit() {
    ...
    assert!(
        matches!(replace, Err(lisp_sitter_core::Error::MalformedSource(_))),
        "{replace:?}"
    );

    let remove = transform::remove_form(&reg, path.to_str().unwrap(), "b", true);
    assert!(
        matches!(remove, Err(lisp_sitter_core::Error::MalformedSource(_))),
        "{remove:?}"
    );

    assert!(
        read().contains("(defun c () 3)"),
        "form c must not be deleted"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn stdin_path_outputs_only_result() {
    ...
    assert_eq!(
        run(&["check-node", "--lang", "elisp", "--body-file", "-"]),
        "OK"
    );
}
...
fn rename_project_updates_definition_and_all_callers() {
    ...
    assert!(
        a_out.contains("(defun square (x)"),
        "definition renamed: {a_out}"
    );
    assert!(
        a_out.contains("(square 3)"),
        "same-file caller renamed: {a_out}"
    );
    assert!(!a_out.contains("helper"), "no stale references: {a_out}");

    let b_out = std::fs::read_to_string(&b).unwrap();
    assert!(
        b_out.contains("(square 5)"),
        "cross-file caller renamed: {b_out}"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn rename_project_errors_when_symbol_undefined() {
    let reg = reg();
    let dir = std::env::temp_dir().join(format!(
        "lisp-sitter-integ-{}-rename-missing",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    ...
    assert!(
        matches!(r, Err(lisp_sitter_core::Error::FormNotFound(_))),
        "{r:?}"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn analyze_reports_unused_arity_and_unresolved() {
    let reg = reg();
    let dir =
        std::env::temp_dir().join(format!("lisp-sitter-integ-{}-analyze", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let p = dir.join("a.el");
    std::fs::write(
    ...
    assert!(
        report.contains("unused") && report.contains("lonely"),
        "{report}"
    );
    assert!(
        report.contains("arity") && report.contains("`add`"),
        "{report}"
    );
    assert!(
        report.contains("unresolved") && report.contains("mystery"),
        "{report}"
    );
...
fn analyze_suppresses_autoloaded_and_required() {
    let reg = reg();
    let dir =
        std::env::temp_dir().join(format!("lisp-sitter-integ-{}-analyze2", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
