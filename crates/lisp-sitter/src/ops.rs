...
pub fn resolve_plugin<'a>(reg: &'a Registry, path: &str, lang: Option<&str>) -> Result<&'a dyn lisp_sitter_core::LanguagePlugin, Error> {
    ...
    match lang {
        ...
        None => {
            if path == "-" {
                return Err(Error::Message("provide --lang when reading from stdin".into()));
            }
...
pub fn find_errors(reg: &Registry, path: &str) -> Result<String, Error> {
    let c = read_file(path)?; let errs = resolve_plugin(reg, path, None)?.find_errors(&c);
    if errs.is_empty() { Ok("No errors".to_string()) } else { Ok(errs.join("\n")) }
}
...
pub fn fmt_write(reg: &Registry, path: &str) -> Result<String, Error> {
    let c = read_file(path)?; let p = resolve_plugin(reg, path, None)?;
...
pub fn atomic_write(path: &str, content: &str) -> Result<(), Error> {
    if path == "-" {
        return Err(Error::Message("cannot --write when reading from stdin".into()));
    }
    let p = Path::new(path);
...
pub fn callers(reg: &Registry, path: &str, sym: &str) -> Result<String, Error> {
    ...
    for sr in p.find_symbol_refs(&c, sym) {
        if sr.kind != lisp_sitter_core::RefKind::CallHead { continue; }
        let pos = sr.form_start;
        if def.is_some_and(|(ds, de)| pos >= ds && pos < de) { continue; }
...
mod tests {
    ...
    fn test_fmt_write_malformed_source_refused() {
        ...
        assert!(matches!(result, Err(lisp_sitter_core::error::Error::MalformedSource(_))),
            "fmt_write should refuse malformed source: {:?}", result);
        assert_eq!(std::fs::read_to_string(&path).unwrap(), original, "file must not be modified");
        let _ = std::fs::remove_dir_all(&dir);
    }
