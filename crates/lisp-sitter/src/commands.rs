...
pub fn init_git_hook() -> Result<()> {
    ...
    if !hook_path.parent().map(|p| p.exists()).unwrap_or(false) {
        anyhow::bail!("no .git/hooks -- in a git repo?");
    }
    let hook = r#"#!/bin/sh
...
mod tests {
    ...
    fn test_check_valid_file() {
        ...
        let dir =
            std::env::temp_dir().join(format!("lisp-sitter-cmd-test-check-{}", std::process::id()));
    ...
    fn test_wrap_body() {
        ...
        let reg = crate::default_registry();
        let dir =
            std::env::temp_dir().join(format!("lisp-sitter-cmd-test-wrap-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("test.el");
    ...
    fn test_check_semantic_no_warnings() {
        let reg = crate::default_registry();
        let dir =
            std::env::temp_dir().join(format!("lisp-sitter-cmd-test-sem-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("test.el");
        std::fs::write(&path, "(defun foo () 1)\n").unwrap();
    ...
    fn test_init_git_hook_in_temp_repo() {
        let dir =
            std::env::temp_dir().join(format!("lisp-sitter-cmd-test-hook-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
