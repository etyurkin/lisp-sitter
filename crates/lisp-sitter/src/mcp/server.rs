...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
struct PathDepthArgs { path: String, #[serde(default)] depth: Option<i64> }
...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
struct PatternArgs { path: String, symbol: String, pattern: String, #[serde(default)] write: bool }
...
impl LispSitterMcp {
    ...
    #[tool(description = "List MISSING tokens and ERROR nodes in a Lisp file as reported by tree-sitter. Use this to locate missing parentheses or other structural problems without reading the whole file.")]
    async fn structural_find_errors(&self, Parameters(args): Parameters<PathArgs>) -> Result<String, String> { tool_result(ops::find_errors(&self.reg, &args.path)) }
    ...
    async fn structural_replace(&self, Parameters(args): Parameters<ReplaceArgs>) -> Result<String, String> {
        if args.diff { let c = ops::read_file(&args.path).map_err(|e| e.to_string())?; let p = ops::resolve_plugin(&self.reg, &args.path, None).map_err(|e| e.to_string())?;
    ...
    async fn structural_format(&self, Parameters(args): Parameters<FormatArgs>) -> Result<String, String> {
        let c = ops::read_file(&args.path).map_err(|e| e.to_string())?; let f = ops::format_content(&c, &self.reg, &args.path).map_err(|e| e.to_string())?; let mut r = String::new();
        if args.diff { let d = ops::diff_text(&c, &f, &args.path); if !d.is_empty() { r.push_str(&d); } }
        if args.write { let p = ops::resolve_plugin(&self.reg, &args.path, None).map_err(|e| e.to_string())?; lisp_sitter_core::edit::ensure_source_editable(p, &c).map_err(|e| e.to_string())?; p.check_file(&f).map_err(|e| e.to_string())?; ops::atomic_write(&args.path, &f).map_err(|e| e.to_string())?; r.push_str(&format!("Wrote {}", args.path)); } else { r.push_str(&f); } Ok(r)
    }
    ...
    #[tool(description = "Paredit splice: remove a wrapper form, elevating its body into the \
    ...
    async fn structural_splice(&self, Parameters(args): Parameters<PatternArgs>) -> Result<String, String> {
        let u = lisp_sitter::transform::splice(&self.reg, &args.path, &args.symbol, &args.pattern).map_err(|e| e.to_string())?;
        if args.write { ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?; Ok(format!("Wrote {}", args.path)) } else { Ok(u) }
    }

    #[tool(description = "Paredit raise: replace the direct enclosing list with a chosen \
        sub-expression. \
        Example: `(if cond (bar x) nil)` raise `(bar x)` → `(bar x)`. \
        Pass write=true to save.")]
    async fn structural_raise(&self, Parameters(args): Parameters<PatternArgs>) -> Result<String, String> {
        let u = lisp_sitter::transform::raise(&self.reg, &args.path, &args.symbol, &args.pattern).map_err(|e| e.to_string())?;
        if args.write { ops::atomic_write(&args.path, &u).map_err(|e| e.to_string())?; Ok(format!("Wrote {}", args.path)) } else { Ok(u) }
    }
    ...
    #[tool(description = "Inline all call sites of a function with its body and remove the definition.")]
...
mod tests {
    ...
    async fn test_structural_find_errors_clean() {
        ...
        let r = s.structural_find_errors(Parameters(PathArgs { path: p.to_str().unwrap().to_string(), semantic: false })).await;
    ...
    async fn test_structural_find_errors_missing_paren() {
        ...
        let r = s.structural_find_errors(Parameters(PathArgs { path: p.to_str().unwrap().to_string(), semantic: false })).await;
        let out = r.unwrap();
        assert!(out.contains("missing"), "expected missing paren report, got: {out}");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    async fn test_structural_rename() {
        ...
        let r = s.structural_rename(Parameters(RenameArgs { refs: false, no_refs: false,
            path: p.to_str().unwrap().to_string(),
            old: "foo".into(),
            new: "bar".into(),
            write: false,
