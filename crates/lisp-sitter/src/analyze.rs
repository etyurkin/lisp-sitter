...
fn is_curried_define(form_text: &str) -> bool {
    let t = form_text.trim_start();
    let Some(t) = t.strip_prefix('(') else { return false };
    let t = t.trim_start();
    let rest = t.strip_prefix("define").unwrap_or(t);
...
/// Quoted and quasiquoted data are skipped (not scanned for calls).
fn scan_calls(content: &str, dialect: Dialect) -> Vec<Call> {
    let mut out = Vec::new();
...
fn scan_range(content: &str, b: &[u8], mut i: usize, end: usize, d: Dialect, out: &mut Vec<Call>) {
    while i < end {
        ...
        if b[i] == b'(' {
            ...
            // any non-list datum (symbol, string, char, number, quoted form)
            let n = skip_sexp_in(b, i, d).unwrap_or(i + 1);
            i = if n > i { n } else { i + 1 };
        }
    }
...
/// head included.
fn list_children(b: &[u8], open: usize, close: usize, d: Dialect) -> Vec<(usize, usize)> {
...
/// Record the list `[open, close)` as a call (when its head is a plain symbol)
/// and recurse into its children. Binding positions — function parameter lists,
/// `lambda`/`define` signatures, and `let`-family binding variables — are NOT
/// treated as calls, which removes the most common class of false positives.
fn scan_list(content: &str, b: &[u8], open: usize, close: usize, d: Dialect, out: &mut Vec<Call>) {
    ...
    match head {
        // (defun NAME (PARAMS) BODY…) — skip NAME and the parameter list.
        Some(h) if is_fn_def_head(h) => recurse(kids.get(3..).unwrap_or(&[]), out),
        // (lambda (PARAMS) BODY…)
        Some("lambda") => recurse(kids.get(2..).unwrap_or(&[]), out),
        // (define (SIG) BODY…) or (define NAME VALUE) — skip the name/signature.
        Some("define") => recurse(kids.get(2..).unwrap_or(&[]), out),
        // (let ((VAR INIT)…) BODY…) — recurse INIT expressions and the body, not VARs.
        ...
        // ordinary list: recurse into every child (head included, for ((lambda …) …) cases).
        _ => recurse(&kids, out),
    }
}
...
mod tests {
    ...
    fn scan_counts_args_and_skips_quotes() {
        ...
        assert_eq!(foo.argc, 4); // a, b, (bar c), '(...)
